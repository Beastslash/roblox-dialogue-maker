...
function API.Gui.CreateNewDialogueGui(theme)
	...
	if not DialogueGui then
		...
		if not DialogueGui then
			error("[Dialogue Maker] Default theme \""..API.Gui.GetDefaultThemeName().."\" couldn't be found in the themes folder.");
		end;
	end;
...
function API.Triggers.AddProximityDetector(npc, proximityDetector)
...
function API.Triggers.DisableAllProximityDetectors()
...
function API.Triggers.EnableAllProximityDetectors()
	for _, proximityDetector in pairs(ProximityDetectors) do
